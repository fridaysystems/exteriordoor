				...
				<div class="comment-author vcard">
					...
					<?php if ( ! $has_edit_cap && $can_edit_note ) : ?>
						&mdash; <span class="comment-author-edit-link">
							...
							<a class="comment-edit-link" href="<?php echo site_url( "docs/reference/comment/edit/{$comment->comment_ID}" ); ?>"><?php _e( 'Edit', 'wporg' ); ?></a>
						</span>
					<?php endif; ?>
...
function get_params( $post_id = null ) {
	...
	if ( $tags ) {
		...
		foreach ( $tags as $tag ) {
			...
			$tag = json_decode( str_replace( array( '<strong>', '</strong>' ), '__', json_encode( $tag ) ) );
...
function get_source_file_link( $post_id = null, $line_number = true ) {
	...
	if ( ! empty( $source_file ) ) {
		$url = apply_filters( 'wp-parser_source_url_base', 'https://core.trac.wordpress.org/browser/tags/' . get_current_version() . '/src/', $post_id ) . $source_file;
		// Line number.
...
function get_source_code( $post_id = null, $force_parse = false ) {
	...
	if ( ! $force_parse && $source_code = get_post_meta( $post_id, $meta_key, true ) ) {
		error_log( 'source code stored in post meta' );
		return $source_code;
	}
